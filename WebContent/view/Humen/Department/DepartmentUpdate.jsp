...
<body>
	<form action="DepartmentUpdateProc.htm?deno=<%=request.getParameter("deno")%>&emno=<%=request.getParameter("emno")%>" method="post">
					...
					<div class="jumbotron">
						<table class="table table-striped">
							...
							<c:forEach var="n" items="${departments}">
								<tr>
									<th>${n.deno}</th>
									<th><a href="DepartmentUpdateContent.htm?deno=${n.deno}&emno=<%=request.getParameter("emno")%>">${n.dename}</a></th>
									<th>${n.detel1}</th>
									<th>${n.detel2}</th>
									<c:if test="${n.emno < 0}">
										<th>없음</th>
