...
<%
	...
	String query1 = "SELECT * FROM estimate  ORDER BY eid asc;";
...
<body>
	<form action="estimatedeleteProc.jsp?deno=<%=request.getParameter("deno")%>&emno=<%=request.getParameter("emno")%>" method="post">
		...
		<div class="container">
			...
			<div class="row row-offcanvas row-offcanvas-right">
				<div class="col-xs-6 col-sm-3 sidebar-offcanvas" id="sidebar"
					role="navigation">
					<jsp:include page="/WEB-INF/view/inc/sales/estimate/left.jsp"/>
					<!--/.well -->
				</div>
				<!--/span-->
				<div class="col-xs-12 col-sm-9">
					...
					<div class="jumbotron">
						<table class="table table-striped">
							...
							%>
							<tr>
								<th><input type="checkbox" name="eid"
									id="<%=rs.getString("eid")%>"
									value="<%=rs.getString("eid")%>" /><label
									for="<%=rs.getString("eid")%>"></label></th>
								<th><%=rs.getString("sname")%></th>
								<th><%=rs.getString("pno")%></th>
								<th><%=rs.getString("pname")%></th>
								<th><%=rs.getString("enum")%></th>
								<th><%=rs.getString("price")%></th>
								<th><%=rs.getString("esum")%>원</th>
							</tr>
							<%
								}
								rs.close();
